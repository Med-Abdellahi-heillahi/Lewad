...
const ledgerRlsPath = new URL(
  "../supabase/migrations/20260819000000_phase_db1_profiles_wallets_ledger.sql",
  import.meta.url,
);
const searchRlsPath = new URL(
  "../supabase/migrations/20260819000002_phase_db3a_secure_search_credit_debit.sql",
  import.meta.url,
);
const rechargeRlsPath = new URL(
  "../supabase/migrations/20260820000001_recharge_requests_admin_approval.sql",
  import.meta.url,
);
const rechargeRpcPath = new URL(
  "../supabase/migrations/20260820000003_create_recharge_request_rpc.sql",
  import.meta.url,
);
const submissionRlsPath = new URL(
...
function dataLayer() {
  return readFileSync(dataLayerPath, "utf8").replaceAll("\r\n", "\n");
}
...
describe("user history contracts", () => {
  ...
  it("leans on server-side row scoping rather than a client-side user filter", () => {
    ...
    expect(readFileSync(searchRlsPath, "utf8")).toContain(
      "grant select on public.search_logs to authenticated;",
    );
    expect(readFileSync(ledgerRlsPath, "utf8")).toContain(
    ...
    expect(readFileSync(rechargeRlsPath, "utf8")).toContain(
      "grant select, insert on public.recharge_requests to authenticated;",
    );
    expect(readFileSync(rechargeRpcPath, "utf8")).toContain(
  ...
  it("counts a paid search once, not twice", () => {
    ...
    expect(source).toMatch(/\.neq\(["']type["'],\s*["']search_debit["']\)/);
  ...
  it("shows suggestions as nothing at all, because they cost nothing", () => {
    ...
    const suggestions = readFileSync(
      new URL("../src/lib/searchSuggestions.ts", import.meta.url),
      "utf8",
    );
  ...
  it("carries the status and the 200 MRO / 3-month terms of a submission", () => {
    ...
    expect(source).toMatch(/["']business_submission["']/);
  ...
  it("paginates the client history in ten-item pages", () => {
    const source = page();

    expect(source).toContain("HISTORY_PAGE_SIZE");
    expect(source).toContain("events.slice(start, start + HISTORY_PAGE_SIZE)");
    expect(source).toContain("<PaginationControls");
    expect(source).toContain("totalPages > 1");
    expect(source).not.toContain("showMore");

    const history = readFileSync(
      new URL("../src/lib/userHistory.ts", import.meta.url),
      "utf8",
    );
    expect(history).toContain("export const HISTORY_PAGE_SIZE = 10");
  });

  it("documents the future server-side history pagination boundary", () => {
    const codex = readFileSync(new URL("../CODEX.md", import.meta.url), "utf8");

    expect(codex).toContain("Client history pagination TODO");
    expect(codex).toContain("10 items per page");
    expect(codex).toContain("server-side pagination");
    expect(codex).toContain("authenticated user's own events");
    expect(codex).toContain("admin audit logs");
  });
  ...
  it("never puts internal vocabulary in front of the client", () => {
    ...
    for (const path of localePaths) {
      ...
      const block = dictionary.slice(
        dictionary.indexOf("  history: {"),
        dictionary.indexOf("  appSearch: {"),
      );
  ...
  it("is reachable from the member navigation, credits, and profile", () => {
    const nav = readFileSync(
      new URL("../src/components/shell/appNav.ts", import.meta.url),
      "utf8",
    ).replaceAll("\r\n", "\n");
    ...
    const pages = readFileSync(
      new URL("../src/components/AppPages.tsx", import.meta.url),
      "utf8",
    );
