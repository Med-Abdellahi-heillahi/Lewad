-- PostgREST resolves browser RPCs by exposed name and JSON argument keys. Keep
-- the typed import entry point uniquely named so it cannot be confused with
-- the legacy one-argument internal implementation.
...
begin
  ...
  -- The legacy one-argument implementation owns the atomic discovery lock,
  -- duplicate check, establishment/main-branch creation, and discovery state.
  -- It remains internal because execute was revoked from browser roles in 00014.
